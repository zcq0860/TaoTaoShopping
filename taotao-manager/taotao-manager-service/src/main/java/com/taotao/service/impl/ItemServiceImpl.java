...
public class ItemServiceImpl implements ItemService {
	...
	@Autowired
	private TbItemParamMapper itemParamTbItemParamMapper;
	@Autowired
	...
	public TaotaoResult createItem(TbItem item, String desc, String itemParam) throws Exception {
	    ...
	    itemCat=itemCatMapper.selectByPrimaryKey(item.getCid());    //因为item中没有商品类目的名字，只有cid，所以需要用cid去查询到类目名称   
	    try {           
	        // 把商品信息写入索引库
	...
	public TaotaoResult updateItemStatus(List<Long> ids, String method) {
		...
		if (method.equals("reshelf")) {
			...
			item.setUpdated(new Date());
			String id=String.valueOf(item.getId());  //因为solrServer的deleteById接受的是String类型的数据，所以需要转换下
